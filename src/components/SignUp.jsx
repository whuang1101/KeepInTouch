...
const SignUp = () => {
  ...
  const onEmailBlur = () => {
    ...
    fetch(`https://red-silence-64.fly.dev/users/email/${newUser.email}`)
  .then((response) => {
  ...
  const onUsernameBlur = () => {
    ...
    fetch(`https://red-silence-64.fly.dev/users/username/${newUser.username}`)
  .then((response) => {
    return response.json()
  ...
  const handleSignUp = (e) => {
    ...
    if(passwordsMatch && verifyEmail && verifyUsername){
      fetch("https://red-silence-64.fly.dev/users",{
        method:"POST",
        headers: {
